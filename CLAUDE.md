...
1. **Portada con fachadas reales — administrada DESDE EL SHEET (hoja `Proyectos`).** Decisión de Alejandro 2026-06-24: nada de residencias/fachadas hardcodeadas en el HTML; el Sheet controla todo. La hoja `Proyectos` (creada/llenada vía doPost `list`) tiene columnas `id,name,sub,active,key,cover,url` y hoy trae las 6 residencias Aurum con su fachada (las mismas del cuestionario de Arquitectura de Autor): rnm=Residencia Navarro Muñoz (`active:sí`, clave Mona, `fachada-mona.jpg`), aria=Casa Aria, maria=Casa María (`fachada-minimalista.jpg`), zara=Casa Zara (`fachada-mediterraneo.jpg`), **alysa=Casa Alysa** (`fachada-industrial.jpg` — en el cuestionario esa fachada se etiqueta "Barcelona/Industrial", pero la residencia real es Alysa), rita=Casa Rita (`fachada-clasico.jpg`). Las 5 sin board van `active:no` ("Próximamente", atenuadas a opacity .6). El `cover` es la URL pública de la imagen (hoy `https://alexpueblag.github.io/interiores-aurum/img/fachada-*.jpg`; cambiable a link de Drive editando la celda). `imgUrl()` deja pasar tal cual lo que no es Drive. El `var PROJECTS` del HTML quedó como salvavidas mínimo (solo rnm) por si el Sheet no responde — NO agregar residencias ahí. Las 6 imágenes viven en `img/` del repo como hosting estable de los `cover`. **Claves (`key`) = nombre invertido temporal (rnm=Mona; aria=aira, maria=airam, zara=araz, alysa=asyla, rita=atir) — placeholder "por mientras"; Alejandro las cambiará 1×1 cuando quiera privacidad real.** El gate compara en minúsculas con trim (`tryGate`), así que las claves van en minúsculas/sin acentos. Las 6 quedaron `active:sí` por decisión de Alejandro 2026-06-24 (le advertí que, al haber un solo dataset, entrar a cualquiera con su clave muestra los interiores de Navarro Muñoz bajo ese nombre hasta que cada una tenga su propio board vía la columna `url`). Todo editable en la hoja Proyectos.
